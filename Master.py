...
import math
# %%
Data_Name="China infection.csv"
Use_Percent = True
if Use_Percent:
    train_Percent = 0.80
else:
    Testing_Rows = 7
Forecasting_days = 12 # how many days that you want to forecast?
create_new_model = True
Time="Months"
Implementation="Energy consumed in GWh in Japan "
plot_title="Modelling and forecasting Energy consumed in GWh by using GRU Model"
Model_Name="gru"
# %%
...
RRMSE_train = trainScore_RMSE/statistics.mean(trainY[0])
print('Train Score: %.2f RRMSE' % (RRMSE_train))
RRMSE_test= testScore_RMSE/statistics.mean(testY[0])
print('Test Score: %.2f RRMSE' % (RRMSE_test))
# calculate Pearson's correlation
trainScore_correlation = pearsonr(trainY[0], trainPredict[:,0])
print('Train Score: %.2f correlation ' % (trainScore_correlation[0]))
testScore_correlation= pearsonr(testY[0], testPredict[:,0])
print('Test Score: %.2f correlation ' % (testScore_correlation[0]))
# calculate Mean Bias Error(MBE) for a set of actual and test prediction
MBE_train = np.mean(trainPredict[:,0] - trainY[0]) #here we calculate MBE
print('Train Score: %.2f Mean Bias Error ' % (MBE_train))
MBE_test = np.mean(testPredict[:,0] - testY[0]) #here we calculate MBE
print('Test Score: %.2f Mean Bias Error ' % (MBE_test))
#%%
